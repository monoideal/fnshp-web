...
export default function ViewCatalogOwnedTable() {
  const api = useApi();
  ...
  const getBooks = async pageNum => {
    const res = await api.fetchCreatorBooks(
      pageNum,
      rowsPerPage,
      orderBy,
      order,
  ...
  const handleBookSelection = (id, index) => {
  ...
  const handleClick = row => {
    history.push(`/catalog/view/${row.id}`);
  };
  ...
  const fields = [
    ...
    { label: `ON SALES`, name: `onSale`, sortable: true },
  ];
          ...
          <Table className={classes.table}>
            ...
            {isLoading ? (
              ...
              books.map((row, index) => (
                <TableBody key={index}>
                  <TableRow
                    onClick={() => handleBookSelection(row.id, index)}
                    className={classes.tableRowHover}
                  >
                    <TableCell align="left" className={classes.img}>
                      <Grid container>
                        <Grid item xs={6}>
                    ...
                    <TableCell onClick={() => handleClick(row)} align="left">
                      {row.contributors.map(a => a.displayName).join(', ')}
                    </TableCell>
                    <TableCell onClick={() => handleClick(row)} align="left">
                    ...
                    <TableCell onClick={() => handleClick(row)} align="left">
                      <img
                        ...
                        src={
                          isNil(row.status)
                            ? ''
                            : `/${row.status.toLowerCase()}.svg`
                        }
                      />
                    </TableCell>
                    <TableCell onClick={() => handleClick(row)} align="center">
                      {row.onSale ? (
